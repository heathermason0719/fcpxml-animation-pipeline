...
function renderCommentList(target,comments,{showTime=false}={}){target.replaceChildren();if(!comments.length){target.append(node('div','muted','暂无 comment'));return}for(const comment of comments){const item=node('article','comment-item');const meta=node('div','comment-meta');const anchor=showTime&&comment.timeStart?`${formatTime(sec(comment.timeStart))}${comment.timeEnd?`–${formatTime(sec(comment.timeEnd))}`:''} · `:'';meta.append(node('span','',`${anchor}${scopeText(comment)}`),node('span',`badge ${comment.status}`,comment.status));item.append(meta,node('p','',comment.body));target.append(item)}}
...
byId('showStoryboard').onclick=()=>switchView('storyboard');byId('showDemo').onclick=()=>switchView('demo');byId('refresh').onclick=load;byId('demo').ontimeupdate=()=>updatePlayerContext();byId('useRange').onchange=event=>{byId('rangePanel').hidden=!event.target.checked;if(!event.target.checked){rangeStartValue=null;rangeEndValue=null;byId('rangeStart').textContent='未设置';byId('rangeEnd').textContent='未设置'}};byId('captureStart').onclick=()=>{rangeStartValue=byId('demo').currentTime;byId('rangeStart').textContent=formatTime(rangeStartValue)};byId('captureEnd').onclick=()=>{rangeEndValue=byId('demo').currentTime;byId('rangeEnd').textContent=formatTime(rangeEndValue)};
    ...
    class ReviewHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if parsed.path == "/api/state":
                self._json(200, review_state(root))
                return
