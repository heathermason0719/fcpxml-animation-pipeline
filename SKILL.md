...
Record the resulting route in the draft manifest and expose it through the existing A11 storyboard review together with real copy and static keyframes. Do not add a per-cue approval step. Ask separately when a branch would materially change scope, violate an approved constraint, or create a hard-to-reverse consequence. Also stop before choosing or rendering an A11 outcome when the user's wording or accumulated confirmed constraints still support two or more reasonable interpretations that would materially change the final composition, required content, motion behavior, or special constraints; ask one focused clarification question instead of selecting an interpretation for the user. Low-cost reversibility and the ability to comment later in A11 do not make such ambiguity Agent-owned. Do not ask when the outcome is unambiguous and only technical implementation details remain. Once clarified, continue without adding another approval gate. Do not copy the cross-project grammar into project `frame.md`; that file records only the current video's approved visual package.
