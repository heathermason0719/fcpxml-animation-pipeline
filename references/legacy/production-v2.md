...
Do not change a closed Vn through a controlled writer. When the user identifies a concrete affected animated cue and scope, use the formal linear rework entrypoint:

```bash
python3 <skill-directory>/scripts/rework_lifecycle.py begin "/absolute/project/workspace/AfterForge/YYYY-MM-DD_Vn" \
  --change "cue_id:motion" --reason "具体返工原因"
python3 <skill-directory>/scripts/rework_lifecycle.py verify "/absolute/project/workspace/AfterForge/YYYY-MM-DD_Vn"
```

`--change` may be repeated; each cue uses `static`, `motion`, or `static,motion`. The entrypoint archives the preceding closed round under `rework/history/rNNNN/` without changing its historical schema. A motion-only rework retains valid A11 approvals; a static rework invalidates only the named cue approvals. Every new round discards A12–A14 and D-stage authority, creates a fresh complete Demo, and requires the user's new A13 approval and A14 authorization before native rendering. Revision 0 keeps `delivery/`; later rounds use `delivery/revisions/rNNNN/` and receive distinct package/Event/Project identities. No incremental render, rollback UI, or inferred user approval is provided.
...
The registrar is the only delivery step allowed to consume the current revision ledger: `delivery/render-ledger.json` for revision 0, or `delivery/revisions/rNNNN/render-ledger.json` after rework. It verifies every animated MOV again, records stable `deliveryAsset` data in the main manifest, and leaves every `source-only` cue unregistered. Registration must not invalidate or rewrite A11 layout locks.
